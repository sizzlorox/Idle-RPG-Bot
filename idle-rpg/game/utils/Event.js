...
class Event {
    ...
    return new Promise((resolve) => {
      ...
      if (dropitemChance <= 15 + (selectedPlayer.stats.luk / 2)) {
        return this.ItemManager.generateItem(selectedPlayer, mob)
          .then((item) => {
            events.utils.dropItem(this.ItemManager, selectedPlayer, item);
    ...
    return new Promise((resolve) => {
      return this.ItemManager.generateItem(selectedPlayer)
        .then((item) => {
          ...
          events.utils.townItem(this.ItemManager, selectedPlayer, item, itemCost, resolve);
  ...
  generateItemEventMessage(selectedPlayer, item) {
    ...
    return events.utils.randomItemEventMessage(randomEventInt, selectedPlayer, item);
  }
  ...
  stealPlayerItem(discordHook, twitchBot, stealingPlayer, victimPlayer) {
    return new Promise((resolve) => {
      ...
      if (luckStealChance > (90 - canSteal)) {
        ...
        events.utils.stealEquip(this.ItemManager, discordHook, stealingPlayer, victimPlayer, itemKeys[luckItem]);
