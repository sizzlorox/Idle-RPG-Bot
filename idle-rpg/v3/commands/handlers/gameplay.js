...
module.exports = [
  {
    ...
    operatorOnly: false,
    channelOnly: false,
    handler: async ({ game, bot, message, author }) => {
      ...
      const actionsChannel =
        playerGuild &&
        playerGuild.channels.cache.find(
      ...
      switch (spell) {
        case 'bless': {
          ...
          if (player.gold.current >= globalSpells.bless.spellCost * calcAmount && calcAmount >= 1) {
            ...
            if (actionsChannel) {
              actionsChannel.send(
                setImportantMessage(
                  `${player.name}${player.titles.current !== 'None' ? ` the ${player.titles.current}` : ''} just cast${calcAmount > 1 ? ` ${calcAmount}x ` : ' '}${spell}!!\nCurrent Active Bless: ${result.spells.bless.reduce((prev, curr) => prev + curr.count, 0)}\nCurrent Multiplier is: ${result.multiplier}x`,
                ),
              );
            }
          } else {
            author.send(
              `You do not have enough gold! This spell costs ${globalSpells.bless.spellCost} gold.`,
            );
          }
          break;
        }
        case 'home': {
          if (player.gold.current >= globalSpells.home.spellCost) {
            ...
            author.send(
              `You do not have enough gold! This spell costs ${globalSpells.home.spellCost} gold.`,
            );
          }
          break;
        }
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      const lotteryChannel =
        lotteryGuild && lotteryGuild.channels.cache.get(enumHelper.channels.lottery);
      if (lotteryChannel) {
        let lotteryMessages = await lotteryChannel.messages.fetch({ limit: 10 });
        lotteryMessages = lotteryMessages.sort(
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      if (!loadedPlayer || !loadedPlayer.quest)
        return author.send("I'm sorry but you have no quest.");
      ...
      return author.send(
        `Quest ${oldQuestMob} has been changed to ${updatedPlayer.quest.questMob.name}\nCount: ${updatedPlayer.quest.questMob.count}`,
      );
    },
  },
  {
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      const mapData = maps
        .map(
          (m) => `${m.name} [${m.coords[0]},${m.coords[1]}] - ${m.type ? m.type.name : 'Unknown'}`,
        )
        .join('\n');
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      const value = args[1] ? args[1].trim().toLowerCase() : null;
      const validOptions = [
        enumHelper.pmMode.on,
        enumHelper.pmMode.off,
        enumHelper.pmMode.filtered,
      ];
      if (!value || !validOptions.includes(value)) {
        return author.send(
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      if (args.length < 2 || !args[1]) {
        return author.send(
          'Please input a server ID after the command. (ex: `!setserver 11111111`)',
        );
      }
