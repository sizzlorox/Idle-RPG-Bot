...
async function dailyLottery(bot, game) {
  ...
  for (const guild of bot.guilds.cache.values()) {
    try {
      ...
      const eventMsg = setImportantMessage(
        `Out of ${guildLotteryPlayers.length} contestants, ${winner.name} has won the daily lottery of ${prizePool} gold!`,
      );
      ...
      for (const player of guildLotteryPlayers) {
        ...
        try {
          if (player.discordId !== winner.discordId) {
            await discordUser.user.send(
              `Thank you for participating in the lottery! Unfortunately ${winner.name} has won the prize of ${prizePool} out of ${guildLotteryPlayers.length} people.`,
            );
          } else {
            await discordUser.user.send(
              `Thank you for participating in the lottery! You have won the prize of ${prizePool} out of ${guildLotteryPlayers.length} people.`,
            );
          }
        } catch (_) {
